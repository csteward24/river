...
function update_USGS(connection){
    ...
    fetch(url).then(x => {return x.json()}).then(
        data => {
            var vals = [];
            data.value.timeSeries.forEach(x => {vals.push([x.sourceInfo.siteName,x.values[0].value[0].value,x.values[0].value[0].dateTime])});
            const sql = "INSERT INTO temp(location,value,time) VALUES ?";
